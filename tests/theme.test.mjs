...
test("the interface follows the black red and white design system", async () => {
  ...
  assert.match(html, /id="ally-count">5<\/b>\s*ALLIES<\/span>/);
  assert.match(html, /id="enemy-count">4<\/b>\s*ENEMIES<\/span>/);
  ...
  assert.match(html, /id="panel-unit-count">5<\/span>\s*\/ 5/);
  ...
  assert.match(css, /\.commander-panel\s*\{[^}]*width:\s*min\(540px, calc\(100vw - 28px\)\);[^}]*border:\s*3px solid var\(--white\)/s);
