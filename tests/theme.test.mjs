...
test("the interface follows the black red and white design system", async () => {
  ...
  assert.match(html, /src="\.\/src\/app\.js\?v=16"/);
  ...
  assert.match(html, /id="restart"[^>]*>BATTLE DEMO<\/button>/);
  ...
  assert.match(html, /id="confirm-invasion"[^>]*>START BATTLE<\/button>/);
  assert.match(html, /id="cancel-invasion"[^>]*>CANCEL<\/button>/);
  ...
  assert.match(css, /\.battle-result,\s*\.invasion-dialog\s*\{[^}]*border:\s*4px solid var\(--white\)/s);
