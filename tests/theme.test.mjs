...
test("the interface follows the black red and white design system", async () => {
  ...
  assert.match(html, /id="ally-count">9<\/b>\s*ALLIES<\/span>/);
  assert.match(html, /id="enemy-count">8<\/b>\s*ENEMIES<\/span>/);
  ...
  assert.match(html, /id="panel-unit-count">9<\/span>\s*\/ 9/);
  ...
  assert.match(css, /\.hud-panel\s*\{[^}]*cursor:\s*move/s);
